...
class HINER(nn.Module):
    def __init__(self, args):
        ...
        if 'hiner' in args.arch:
            ...
            raise ValueError('The architecture of model is not applied')
    ...
    def forward(self, img, norm_id, input_embed=None, encode_only=False):
        if 'nerv' in self.arch:
            ...
            img_embed = self.encoder(img).contiguous().view(1,16,self.fc_h_arg,self.fc_w_arg)
        elif 'hnerv' in self.arch:    
            img_embed = self.encoder(img)
        else:
            raise ValueError('The architecture of model is not applied')
